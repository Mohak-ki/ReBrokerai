  ...
  // --- MOBILE FULL MENU DRAWER ---
    ...
    drawer.innerHTML = `
      ...
      <div class="drawer-header-brand" style="background:#0f172a;color:#ffffff;padding:18px 20px;border-bottom:1px solid rgba(255,255,255,0.1);display:flex;align-items:center;justify-content:space-between;">
        ...
        <button class="drawer-close-btn" id="drawer-close-btn" style="background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.25);color:#ffffff;width:32px;height:32px;border-radius:8px;cursor:pointer;font-size:15px;font-weight:800;">✕</button>
      </div>

      <!-- DRAWER BODY (CRISP BLACK WORDS FOR MAXIMUM READABILITY) -->
      <div class="drawer-body" style="padding:18px 16px;display:flex;flex-direction:column;flex:1;overflow-y:auto;background:#ffffff;color:#000000;">
        ...
        <div class="drawer-section-label" style="font-size:12px;font-weight:850;color:#000000;text-transform:uppercase;letter-spacing:0.06em;margin:0 0 8px 6px;">Workspace Navigation</div>
        <nav class="drawer-nav-list" style="display:flex;flex-direction:column;gap:5px;margin-bottom:18px;">
          ${getNavItems().map(([id, iconName, label]) => `
            <a class="drawer-nav-item ${state.page === id ? 'active' : ''}" href="#/${id}" style="display:flex;align-items:center;gap:12px;padding:11px 14px;border-radius:10px;font-size:14.5px;font-weight:750;color:${state.page === id ? '#1d4ed8' : '#000000'};background:${state.page === id ? '#eff6ff' : 'transparent'};border:${state.page === id ? '1.5px solid #bfdbfe' : '1px solid transparent'};text-decoration:none;transition:all 0.15s ease;">
              <span class="drawer-nav-icon" style="color:${state.page === id ? '#1d4ed8' : '#000000'};display:flex;align-items:center;justify-content:center;width:20px;">${svgIcon(iconName, 18)}</span>
              <span style="flex:1;color:${state.page === id ? '#1d4ed8' : '#000000'};font-weight:750;">${label}</span>
              ${state.page === id ? `<span style="font-size:11px;font-weight:850;color:#1d4ed8;background:#dbeafe;padding:2px 8px;border-radius:6px;">Active</span>` : ''}
            </a>
          `).join('')}
        </nav>
        ...
        <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:18px;">
          ...
          ${cap.magicWhatsAppParser ? `
            <button class="drawer-tool-btn" id="drawer-magic-parser-btn" style="display:flex;align-items:center;gap:10px;padding:11px 14px;border-radius:10px;border:1.5px solid #cbd5e1;background:#f8fafc;color:#000000;font-size:14px;font-weight:750;cursor:pointer;text-align:left;">
              <span style="color:#2563eb;">${svgIcon('whatsapp', 18)}</span>
              <span style="color:#000000;font-weight:750;">Magic WhatsApp AI Parser</span>
            </button>
          ` : ''}
          <button class="drawer-tool-btn" id="drawer-cost-btn" style="display:flex;align-items:center;gap:10px;padding:11px 14px;border-radius:10px;border:1.5px solid #cbd5e1;background:#f8fafc;color:#000000;font-size:14px;font-weight:750;cursor:pointer;text-align:left;">
            <span style="color:#0f172a;">${svgIcon('calculator', 18)}</span>
            <span style="color:#000000;font-weight:750;">On-Road Cost & EMI Desk</span>
          </button>
        </div>

        <!-- SECTION: ASSIGNED PACKAGE -->
        <div style="background:#f8fafc;border:1.5px solid #cbd5e1;border-radius:14px;padding:14px;display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
          <div>
            <div style="font-size:11.5px;color:#000000;font-weight:850;text-transform:uppercase;letter-spacing:0.05em;">Assigned Package</div>
            <div style="font-size:14px;font-weight:850;color:#000000;display:flex;align-items:center;gap:6px;margin-top:3px;">
              <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${cap.color};"></span>
              <span style="color:#000000;font-weight:850;">${cap.badge}</span>
            </div>
          </div>
          <a href="#/pricing" class="button secondary" id="drawer-plan-details-btn" style="padding:6px 12px;font-size:12.5px;font-weight:800;color:#000000;text-decoration:none;border-radius:8px;border:1.5px solid #000000;background:#ffffff;">Plan Details →</a>
        </div>

        <!-- SIGN OUT -->
        <div style="margin-top:auto;padding-top:16px;border-top:1.5px solid #e2e8f0;display:flex;justify-content:space-between;align-items:center;">
          <div style="font-size:12.5px;color:#000000;font-weight:800;display:flex;align-items:center;gap:6px;">
            <span style="color:#16a34a;font-size:10px;">🟢</span> Online Cockpit
          </div>
          <button class="button secondary" id="drawer-signout-btn" style="padding:7px 14px;font-size:12.5px;font-weight:800;color:#b91c1c;border:1.5px solid #fca5a5;background:#fef2f2;border-radius:8px;cursor:pointer;">Sign out</button>
        </div>
      </div>
    `;
